...
from core.Image.AutoAdjust import ACE,AWB,ACA
from core.Sharp import Sharp
from core.Blur import Blur
...
class ProThread(QThread):
    ...
    def __init__(self, controller, debug=False, parent=None):
        ...
        self.task_dict = {
                        ...
                        # 'Painter': "",
                        # 'Ink': Ink().ink,
                        "GaussianBlur":Blur.GaussianBlur,
                        "Blur":Blur.Blur,
                        "BlurMore":Blur.BlurMore,
                        "MotionBlur":Blur.MotionBlur,
                        "RadialBlur":Blur.RadialBlur,
                        "USM": Sharp.USM,
    ...
    def run(self):
        ...
        while True:
            if len(self._p_queue) == 0:
                ...
                elif target in ['Blur','BlurMore','GaussianBlur','MotionBlur','RadialBlur','SmartBlur','USM','EdgeSharp','SmartSharp','AWB','ACA','ACE']:
                    if target in self.task_dict.keys():
                        self.tmp_img.image = self.task_dict[target](self.tmp_img.image)
                else:
                    filter = content['data']['filter']
                    self.task_dict[target](filter)
                ...
                self.__controller.layerStack.updateImg()
                # content['callback']()
                # self.__controller.refreshView()
                self.finished.emit()
    
    def doFilter(self, filter):
        _dict = {
        ...
        new = cv2.imread(TABLE_PATH+'/'+_dict[filter])
